...
class axi4_scoreboard extends uvm_scoreboard;
  ...
  typdef struct{bit[DATA_WIDTH-1:0]data,bit[(DATA_WIDTH/8)-1:0]strobe}DataTransaction;
  //keep track of data obtained 
  typedef DataTransaction dataTransactionQueue[$];
  
 //queue of data transaction
  dataTransactionQueue masterArrayDataQueue[];
  
  dataTransactionQueue slaveArrayDataQueue[];

  apb_master_tx masterWriteAddressQueue[$];
  apb_slave_tx slaveWriteAddressQueue[$];
  apb_master_tx masterReadAddressQueue[$];
  apb_slave_tx slaveReadAddressQueue[$];

  logic[7:0]referenceData[longint];
  //Variable : axi4_master_analysis_fifo
  //Used to store the axi4_master_data
  uvm_analysis_imp_master_write_address#(axi4_master_tx,axi_scoreboard) axi4_master_read_address_analysis_fifo;
  uvm_tlm_analysis_fifo#(axi4_master_tx) axi4_master_read_data_analysis_fifo;
  uvm_analysis_imp_master_read_address#(axi4_master_tx,axi_scoreboard) axi4_master_write_address_analysis_fifo;
  ...
  uvm_analysis_imp_slave_write_address#(axi4_slave_tx,axi_scoreboard) axi4_slave_write_address_analysis_fifo;
  ...
  axi4_slave_agent_cfg axi4_slave_agent_cfg_h;
  ...
  uvm_tlm_fifo #(apb_master_tx) referenceFifo;
  //-------------------------------------------------------
  // Externally defined Tasks and Functions
  //-------------------------------------------------------
  extern function new(string name = "axi4_scoreboard", uvm_component parent = null);
  extern virtual function void build_phase(uvm_phase phase);
  extern function void write_master_write_data(axi_master_tx t);
  extern function void write_slave_write_data(axi_slave_tx t);
  extern function void write_master_write_address(axi_master_tx t);
  extern function void write_master_read_address(axi_master_tx t);
  extern function void write_slave_write_address ( axi_slave_tx t);
  extern function void write_slave_read_address(axi_slave_tx t);
...
function axi_scoreboard :: write_master_write_data(axi_master_tx t);
  DataTransaction tempTransaction;
   tempTrsanction.data = t.wdata[0];
  tempTransaction.strobe = t.wstrb[0];
 
    if(t.wlast == 1) begin 
      masterArrayDataQueue[index1].dataTransactionQueue.push_back(tempTransaction);
      index1++;
      masterArrayDataQueue = new[index1] (masterArrayDataQueue);
    end
    else begin 
      masterArrayDataQueue[index1].dataTransactionQueue.push_back(tempTransaction);
    end
...
function axi_scoreboard :: write_slave_write_data(axi_master_tx t);
  DataTransaction tempTransaction;
   tempTrsanction.data = t.wdata[0];
  tempTransaction.strobe = t.wstrb[0];
 
    if(t.wlast == 1) begin 
      slaveArrayDataQueue[index1].dataTransactionQueue.push_back(tempTransaction);
      index1++;
      slaveArrayDataQueue = new[index1] (masterArrayDataQueue);
    end
    else begin 
      slaveArrayDataQueue[index1].dataTransactionQueue.push_back(tempTransaction);
    end
...
function axi_scoreboard :: write_master_write_address(axi_master_tx t);
  masterWriteAddressQueue.push_back(t);
endfunction 

function axi_scoreboard :: write_slave_write_address(axi_slave_tx t);
  slaveWriteAddressQueue.push_back(t);
endfunction 

function axi_scoreboard :: write_master_read_address(axi_master_tx t);
  masterReadAddressQueue.push_back(t);
endfunction 

function axi_scoreboard :: write_slave_read_address(axi_slave_tx t);
  slaveReadAddressQueue.push_back(t);
endfunction 
...
forever begin 
 ...
 int wrapEndAddress;
 axi_master_tx axi_master_address_tx;
 axi_slave_tx axi_slave_address_tx;
  rresp_e readError;
 wait((axi4_master_read_data_analysis_fifo.used()>0 || axi_master_write_response_analysis_fifo.used()>0) &&
      (axi_slave_read_data_analysis_fifo.used()>0 || axi_slave_write_response_analysis_fifo.used()>0) ) begin  
   
   if(axi_slave_write_response_analysis_fifo.used()>0) begin 
     axi_master_write_response_analysis_fifo.get(t);
     axi_slave_write_response_analysis_fifo.get(t1);
     index = slaveWriteAddressQueue.find_first_index() with(item == t1.awid);
     ...
     wrapStartAddress =tempAddress - int'(tempAddress % (((2**(axi_master_address_tx.awisize))* (axi_master_address_tx.awlen +1)));
     wrapEndAddress = wrapStartAddress + (((2**(axi_master_address_tx.awisize))* (axi_master_address_tx.awlen +1)));
 ...
 align =0; 
 alignAmount = axi_master_address_tx.awaddr - (((2**(axi_master_address_tx.awisize))  *(int'(axi_master_address_tx/(2**(axi_master_address_tx.awsize))) =)        
     end
     else begin 
      align=1;
     end 
     for(int i=0;i < masterArrayDataQueue[index].dataTransactionQueue.size();i++) begin 
      int count =0;    
        case(axi_master_address_tx.awburst)
          2'b 00: begin 
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin 
              if(!(tempAddress inside{[axi4_slave_agent_cfg_h.min_address :axi4_slave_agent_cfg_h.max_address]})) begin 
                slave_err = 1;
              end 
              if(masterArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8] != slaveArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8])begin 
                 `uvm_error("BYTE DIFFERENCE","THE BYTE %0D is not equal the byte in expected is %0b and in actual is %0b",j,masterArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8],slaveArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8]);
             
              end    
             referenceFifo.put(masterArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7-:8]);            
            end 
          end  
          2'b 01: begin 
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin     
              ...
              if(masterArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8] != slaveArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7 -: 8])begin 
               ...
               if(masterArrayDataQueue[index].dataTransactionQueue[i].strobe[j]==1)      begin                                       referenceData[tempAddress+count]=(masterArrayDataQueue[index].dataTransactionQueue[i].data[8*j+7-:8]);
end  
             count++;
            end 
            if(align==0)  begin 
              tempAddress = addr+(2**(axi_master_address_tx.awisize))-AlignAmount;
              align=1;
            end
            else 
              tempAddress= tempAddress + (2**(axi_master_address_tx.awisize));
            end           
          end 
          ...
          2'b10:begin 
            ...
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin     
              ...
              if(masterArrayDataQueue[index].dataTransactionQueue[index].data[8*j+7 -: 8] != slaveArrayDataQueue[index].dataTransactionQueue[index].data[8*j+7 -: 8])begin 
                 `uvm_error("BYTE DIFFERENCE","THE BYTE %   0D is not equal the byte in expected is %0b and in actual is %0b",j,masterArrayDataQueue[index].dataTransactionQueue[index].data[8*j+7 -: 8],slaveArrayDataQueue[index].dataTransactionQueue[index].data[8*j+7 -: 8]);
            
              end
              if(masterArrayDataQueue[index].dataTransactionQueue[index].strobe[j]==1)      begin                                       referenceData[tempAddress]=(masterArrayDataQueue[index].dataTransactionQueue[index].data[8*j+7-:8]);
end  
              tempAddress++;
 
              if(tempAddress == wrapLastAddress)
    ...
    if(axi_slave_read_data_analysis_fifo.used()>0) begin 
      axi_master_read_data_analysis_fifo.get(t);
    axi_slave_read_data_analysis_fifo.get(t1);
     readError = new[axi_slave_read_data_analysis_fifo.used()];
     index = slaveWriteAddressQueue.find_first_index() with(item == t1.awid);
        ...
        case(axi_master_address_tx.awburst)
            ...
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin 
             ...
             referenceFifo.get(readCompare);
              if(t1.rdata[0][j] !=readCompare) begin 
                 `uvm_error("ERROR","THE BYTE DOESNT MATCH IN THE POSITION %0d when reference byte is %0d and actual one is %0d",j,readCompare,t1.rdata[0][j])
              end   
              else begin 
                `uvm_info("BYTE MATCH","THE BYTE MATCHES IN POSITION %0d",j);
              end                   
            end 
          end  
          2'b 01: begin 
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin     
              ...
              if(referenceData[tempAddress+count] != t1.rdata[0][j])begin 
                `uvm_error("ERROR","THE BYTE DOESNT MATCH IN THE POSITION %0d when reference byte is %0d and actual one is %0d",j,readCompare,t1.rdata[0][j])
              end   
              else begin 
                `uvm_info("BYTE MATCH","THE BYTE MATCHES IN POSITION %0d",j);
          ...
          2'b10:begin 
            ...
            for(int j=(axi_master_address_tx.awaddr)/(DATA_WIDTH/8);j< (align==0) ? (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize) - (alignAmount)) : (axi_master_address_tx.awaddr +(2**(axi_master_address_tx.awisize);j++) begin     
              ...
              if(referenceData[tempAddress] != t1.rdata[0][j])begin 
                `uvm_error("ERROR","THE BYTE DOESNT MATCH IN THE POSITION %0d when reference byte is %0d and actual one is %0d",j,readCompare,t1.rdata[0][j])
              end   
              else begin 
                `uvm_info("BYTE MATCH","THE BYTE MATCHES IN POSITION %0d",j);
