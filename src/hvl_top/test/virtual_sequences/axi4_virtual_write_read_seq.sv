...
class axi4_virtual_write_read_seq extends axi4_virtual_base_seq;
  ...
  process outstanding_master_wr_seq;
  process outstanding_master_rd_seq;
  process outstanding_slave_wr_seq;
  process outstanding_slave_rd_seq;
...
task axi4_virtual_write_read_seq::body();
  ...
  fork 
    begin: T1_BK_WRITE
      outstanding_master_wr_seq = process::self();
      repeat(2) begin
        axi4_master_bk_write_seq_h.start(p_sequencer.axi4_master_write_seqr_h);
      end
    end
    begin: T2_BK_READ
      outstanding_master_rd_seq = process::self();
      repeat(2) begin
